...
describe.runIf(isDockerAvailable())('steam_user repo', () => {
  ...
  describe('getSteamUser', () => {
    it('returns a user by steam_id', async () => {
      const pool = getPool()
      await insertSteamUser(pool, { steam_id: 'u1', name: 'Alice', solo_mmr: 4000 })

      const result = await steamUser.getSteamUser('u1')
      expect(result).toBeDefined()
      expect(result!.name).toBe('Alice')
      expect(result!.solo_mmr).toBe(4000)
    })
  ...
  describe('saveSteamUser', () => {
    it('upserts a steam user', async () => {
      ...
      expect(result!.solo_mmr).toBe(2000)
    })
  })
  ...
  describe('getSteamUsersMissingMMR', () => {
    it('returns users with 0 MMR registered in a season', async () => {
      ...
      await insertSteamUser(pool, { steam_id: 'zero', name: 'ZeroMMR', solo_mmr: 0, party_mmr: 0 })
      await insertSteamUser(pool, { steam_id: 'nonzero', name: 'HasMMR', solo_mmr: 3000, party_mmr: 2500 })
