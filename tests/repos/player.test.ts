...
describe.runIf(isDockerAvailable())('player repo', () => {
  ...
  async function setupBaseData() {
    ...
    const u1 = await insertSteamUser(pool, { steam_id: 'p1', name: 'Alice', solo_mmr: 4000, party_mmr: 3500 })
    const u2 = await insertSteamUser(pool, { steam_id: 'p2', name: 'Bob', solo_mmr: 3000, party_mmr: 2500 })
